...
public class TradeIngestionService {
    ...
    public TradeIngestionService(
            @Qualifier("ingestionQueue") BlockingQueue<String> ingestionQueue,
    ...
    private void processLoop() {
        while (running && !Thread.currentThread().isInterrupted()) {
            try {
                String xmlPayload = ingestionQueue.take();
                processTradeXml(xmlPayload);
    ...
    private void processTradeXml(String xml) {
        String[] outMessage = {null};
        String[] tradeIdRef = {null};
        ...
        transactionTemplate.executeWithoutResult(status -> {
            try {
                ...
                tradeIdRef[0] = trade.getTradeId();

            } catch (Exception e) {
                log.error("Failed to process trade XML", e);
        ...
        if (outMessage[0] != null) {
            try {
                matchingQueue.put(outMessage[0]);
                log.debug("Trade {} enqueued for matching", tradeIdRef[0]);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }
